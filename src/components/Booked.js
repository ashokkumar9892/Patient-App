...
const Booked = () => {
          ...
          response.data.appointments.map((item, index) => {
            ...
            let obj = {
              ...
              Provider: provider?.find(
                (it) => it.providerid == item.hl7providerid
              )?.displayname,
              date: item.date,
              ...
              Pre: "-",
              Insurance: item.insurances
                ? item.insurances.insuranceplanname
                : "_",
              EB: "_",
              D: "_",
              Copay: item.copay,
  ...
  function filterFromDate(da) {
    ...
    return (
      new Date(da.lastmodified) >= new Date(fromDate) &&
      new Date(da.lastmodified) <= new Date(toDate)
    );
  }
  ...
  useEffect(() => {
    if (sortList === "Booked_Appointments" && bookApptData?.length > 0) {
      ...
      const sortedAsc = data.sort(
        (objA, objB) =>
          Number(new Date(objA.lastmodified).getTime()) -
          Number(new Date(objB.lastmodified).getTime())
      );
      setSortData([...sortedAsc.reverse()]);
    }
  }, [bookApptData]);
  ...
  const checkfromDateLessthantodate = (fromDate, toDate) => {
    ...
    <>
      ...
      <div className="filterDiv">
        <div className="datePicker">
          ...
          <div style={{ marginLeft: "12px" }}>
            <DatePicker
              selected={fromDate}
              onChange={(e) => {
                checkfromDateLessthantodate(e, toDate);
                // setFromDate(e);
              }}
              value={fromDate}
        ...
        <div className="filterdiv">
          ...
          <select
            style={{ width: "172px", height: "28px", marginLeft: "12px" }}
            value={sortList}
            onChange={(e) => {
          ...
          >
            <option
              title="sort by Booked Appointments"
              value="Booked_Appointments"
            >
              {" "}
              Appointment Date
            </option>
            <option title="sort by Booked Date " value="Booked_Date">
              Booked Date
            </option>
          </select>
        </div>
